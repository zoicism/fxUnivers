          ...
          <hr class="hrtitle" style="border-color: #25252533">
			...
			<div class="c3">
			     <div class="rightsidebar">
			     	  <h2 class="title stresstitle">Student</h2>
				  <hr class="hrtitle" style="margin-bottom: 40px;">
				  
                  
          <h3>Purchased Courses (<?php echo $gss_count ?>)</h3>
				  

                  <!-- list purchased courses -->
    <div id="myTakenCourses">
    <table>
    <?php
    if($gss_count>0) {
        $taken_course_id=$first_gss_courseId;
        ...
        $course_link="/userpgs/instructor/course_management/course.php?course_id=".$first_gss_courseId;
        echo '<tr onclick="window.location=\''.$course_link.'\';" style="border-bottom: 1px solid #ccc;cursor:pointer;" class="hoverable"><td style="width: 20%"><img src="/images/artwork/course.png" style="margin-left: 20px; height: 35px; width: 35px;"></td><td style="width: 80%"><strong>'. $gsc_fetch['header'] .'</strong><br>'. $gsc_fetch['description'] .'</td></tr>';
...
<!--
<hr class="hrtitle" style="margin-bottom: 60px">

    <h3>Passed Courses</h3>
    
                  <table>
				    <tr><td><p style="font-size: 7rem; color: #333;"><?php echo $gmac_count ?></p></td></tr>
				    <tr style="height: 90px"><td>Number of Passed courses</td></tr>
				  </table>
-->				    
			     </div>
          ...
          <div class="c33">
          <form method="POST" action="/search/?target=courses">
				    <input type="text" name="search" placeholder="Keywords to search courses" style="width: 100%" required>
                    <button type="submit" class="btn">Search</button>
				  </form>
                  <form method="POST" action="/search/?target=courses">
				    <input type="hidden" name="search" value="" >
                    <button type="submit" class="btn">See All</button>
				  </form>
          </div> 
	</div>
...
<script>
$('#purCourses').click(function() {
    $('#myTakenCourses').toggle();
});
</script>
