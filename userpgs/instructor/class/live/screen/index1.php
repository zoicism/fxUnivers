	...
	<article style="padding:20px;background:white;border: 1px solid black;margin:20px;">
	    ...
	    <section class="experiment" style="flex-flow:column">
                <section class="hide-after-join" style="text-align: center;">                    
                    ...
                    <?php if($user_type=='instructor') echo '<button id="share-screen" style="background-color:transparent" class="submit-btn">Start Screen Sharing</button>'; else echo '<button id="share-screen" style="backgro\
und-color:transparent;display:none;" class="submit-btn">Start Screen Sharing</button><p id="share-screen-p">Instructor\'s screen will appear here.</p>';
...
openSocket: function(config) {
    ...
    //    config.channel = config.channel || location.href.replace(/\/|:|#|%|\.|\[|\]/g, '');
							     config.channel = config.channel || "<?php echo 'fxuniversityscreen'.$class_id ?>";
...
function showPrivateLink() {
  ...
  uniqueToken.innerHTML = '<a href="' + location.href + '" target="_blank" style="display:none">Copy & Share This Private Room Link</a>';
