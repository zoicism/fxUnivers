...
<div class="c3">
      ...
      <form action="/userpgs/instructor/exam/edit_question.php" method="POST" id="question_form">
		        <!-- begin slider area -->
			     <hr class="hrtitle">
			     <h3>Question # <?php echo $numOfQ ?> :</h3>
      <input type="hidden" name="question_id" value="<?php echo $g_c_fetch['question_id'] ?>" >
			<textarea name="question" class="form-control" rows="6" required><?php echo $g_c_fetch['question']?></textarea>

			<hr class="hrtitle" style="border-color:#25252533">
			  <h3>Choices:</h3>
      <p style="font-weight:bold">(A) <input type="text" name="answer_a" style="margin-left: 10px; display: inline; width: 500px;" value="<?php echo $g_c_fetch['option_a']?>"></p>
      <p style="font-weight:bold">(B) <input type="text" name="answer_b" style="margin-left: 10px; display: inline; width: 500px;" value="<?php echo $g_c_fetch['option_b']?>"></p>
      <p style="font-weight:bold">(C) <input type="text" name="answer_c" style="margin-left: 10px; display: inline; width: 500px;" value="<?php echo $g_c_fetch['option_c']?>"></p>
      <p style="font-weight:bold">(D) <input type="text" name="answer_d" style="margin-left: 10px; display: inline; width: 500px;" value="<?php echo $g_c_fetch['option_d']?>"></p>
      <hr class="hrtitle" style="border-color:#25252533">
...
<div class="c3">
      ...
      <div class="list-type1">
			    ...
			    <?php if($get_exam_result->num_rows > 0) {
                        $rowNum=1;
?>
			      <li><a <?php if($q_id==$get_exam_fetch['id']) echo 'style="background-color:#008bc6;color:white;';?> href="/userpgs/instructor/exam/edit_exam.php?q_id=<?php echo $get_exam_fetch['id'].'&course_id='.$get_exam_fetch['course_id'] ?>"><strong><?php echo 'Question #'.$rowNum//$get_exam_fetch['question_id'] ?></strong><br><?php echo $get_exam_fetch['question'] ?></a></li>
			    <?php
			      while($row = $get_exam_result->fetch_assoc()) {
                      $rowNum++;
			    ?>
                      <li><a <?php if($q_id==$row['id']) echo 'style="background-color:#008bc6;color:white;';?>href="/userpgs/instructor/exam/edit_exam.php?q_id=<?php echo $row['id'].'&course_id='.$row['course_id'] ?>"><strong><?php echo 'Question #'.$rowNum//$row['question_id'] ?></strong><br><?php echo $row['question'] ?></a></li>
			    <?php } ?>
<?php } ?>
