...
<h3>Title, Description, Cost</h3><form class="form" id="edit-post" autocomplete="off">
  ...
  <textarea name="description" rows="10" placeholder="Description" required=""><?php echo preg_replace('/\<br(\s*)?\/?\>/i', "",$get_course_fetch['description']) ?></textarea>
				...
				<div class="upload-video-con">
					...
					<p style="" id="up-vid-p">Upload a video from your device.</p>
					<button class="submit-btn" id="up-vid-id" style="">Upload Video</button><form method="POST" style="display:none" id="up-vid-form" action="file_uploader.php" enctype="multipart/form-data">
				    <form>
...
<button class="submit-btn" id="private-invite">Invite</button></div>';*/
			...
			<div class="inner-content-box">
				<div class="add-bulletin-con">
					...
					<form id="bulletin-form">
						<textarea type="text" name="bulletin-body" placeholder="Bulletin text" id="bulletin-txt" required=""></textarea>
		    ...
		    <button onclick="window.location.replace('/userpgs/instructor/course_management/course.php?course_id=<?php echo $course_id ?>')" class="submit-btn done-btn" style="margin: 0 auto;background-color: #00a1e0d4;color: #efefef;font-size: 18px;">Done</button>
		</div>
